...
struct DistrictAdminView: View {
    @Bindable var store: StoreOf<DistrictAdminFeature>
            ...
            Form {
                ...
                Section(header: Text("余興")) {
                    List(store.item.performances) { item in
                        EditableListItemView<Performance>(
                            text: item.name,
                            onEdit: {
