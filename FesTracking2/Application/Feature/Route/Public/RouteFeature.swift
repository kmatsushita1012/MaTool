...
struct RouteFeature{
    ...
    @Reducer
    enum Destination {
        case route(RouteMapFeature)
        case locations(LocationsMapFeature)
    }
    ...
    var body: some ReducerOf<RouteFeature> {
        Reduce{ state, action in
            ...
            case let .routeReceived(routeResult, locationResult):
                ...
                state.map = .route(RouteMapFeature.State(route: route, location: location))
                return .none
            case .locationsReceived(locations: let result):
                switch result {
                case .success(let locations):
                    state.map = .locations(LocationsMapFeature.State(locations: locations))
                case .failure(let error):
                    state.error = error.localizedDescription
                }
                return .none
            case .locationReceived(location: let result):
                ...
                case .success(let value):
                    if case let .route(routeState) = state.map{
                        state.map = .route(RouteMapFeature.State(route: routeState.route, location: value))
                    }
                case .failure(let error):
                    state.error = error.localizedDescription
                }
...
extension RouteFeature.Destination.State: Equatable {}
extension RouteFeature.Destination.Action: Equatable {}

extension RouteFeature.Content: Identifiable,Hashable  {
    var id:String {
        switch self {
        case .locations:
            return "location"
