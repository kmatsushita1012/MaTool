...
- `memo` は `nil`
- `order` は通過順（0始まり）
- 同一地区への再突入は別エントリとして保持する
- ただし同一地区が連続した場合は 1 件に圧縮する（境界ノイズ対策）
...
- 線分ごとに「線分上の交差位置 parameter(t)」を算出し、同一線分内の複数地区通過は route 進行順で並べる
- 再突入は再カウントする
- 最終出力前に連続する同一地区IDを圧縮する
...
- `RouteEditFeature`
  ...
  - 自動判定ボタンの `disabled` 判定は View ではなく Feature (`State` の算出プロパティ) に配置
- `RouteEditView`
  - 自動判定実行ボタン（または同等導線）を追加
  - 通過項目のメニューに「上に追加」「下に追加」を追加し、任意位置への新規挿入を可能にする
...
- 再作成後は、各行のメニューから「上に追加 / 下に追加」で不足項目を途中挿入できる。
...
4. `disabled` 判定が Feature 側プロパティで制御される
5. 任意行の「上に追加 / 下に追加」で指定位置に insert される
...
- 再突入が必要なケースでは再カウントされる
- 同一地区が連続するノイズは1件に圧縮される
- 手動補正として途中挿入（上に追加/下に追加）ができる
