...
struct DistrictControllerTest {
    ...
    @Test func test_getTools_正常() async throws {
        let now = Date()
        let expected = DistrictTool(districtId: "district-id", districtName: "district-name", festivalId: "festival-id", festivalName: "festival-name", milestones: [], base: Coordinate(latitude: 1.0, longitude: 2.0), spans: [Span(id: "s-id", start: now, end: now)])
